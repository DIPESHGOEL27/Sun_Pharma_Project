...
async function uploadMasterAudio(filePath, languageCode, languageName) {
  ...
  form.append('description', `Master voice over audio for ${languageName} language video generation`);
...
async function main() {
  ...
  for (const [fileName, lang] of Object.entries(LANGUAGE_MAP)) {
    ...
    if (fs.existsSync(filePath)) {
      ...
      results.push({ success: false, language: lang.code, error: 'File not found' });
    }
  }
